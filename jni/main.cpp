...
class SFBypassModule : public zygisk::ModuleBase {
public:
    void onLoad(zygisk::ModuleInfo* info) override {
        // Verify we're in surfaceflinger process
        char proc_name[256];
        ...
        if (strcmp(getprogname(), "surfaceflinger") != 0) {
            LOGI("Not in surfaceflinger process, exiting");
            return;
        }

        LOGI("Loading SFBypass module in surfaceflinger process");

        // Initialize whitelist
        initWhitelist();

        // Start inotify thread for whitelist monitoring
        monitorWhitelistFile();

        // Hook the target function
        LOGI("Attempting to hook isCallingBySystemui function...");

        // Try to get the symbol first
        void* symbol_addr = dlsym(RTLD_DEFAULT, target_symbol);
        if (symbol_addr == nullptr) {
            LOGW("Could not find symbol %s, trying pattern search...", target_symbol);
            // Fallback to pattern search would go here
            return;
        }

        // Hook the function using Dobby
        auto result = DobbyHook(symbol_addr, (void*)hooked_isCallingBySystemui,
                                (void**)&original_isCallingBySystemui);

        if (result != 0) {
            LOGE("Failed to hook isCallingBySystemui function");
            return;
        }

        LOGI("Successfully hooked isCallingBySystemui function");
    }
...
REGISTER_ZYGISK_MODULE(SFBypassModule)